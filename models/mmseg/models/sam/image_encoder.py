...
class ImageEncoderViT(nn.Module):
    def __init__(
        ...
        for i in range(depth):
            if self.adapter_tuning and self.adapter_type == 'shallow':
                if i == 0:
                    block = AdapterBlock(
                     dim=embed_dim,
                     num_heads=num_heads,
            ...
            elif self.adapter_tuning and self.adapter_type == 'deep':
                if i <= 2:
                    block = AdapterBlock(
                        dim=embed_dim,
                        num_heads=num_heads,
                    ...
                    self.blocks.append(block)
            elif self.adapter_tuning and self.adapter_type == 'full':
                block = AdapterBlock(
                    dim=embed_dim,
                    num_heads=num_heads,
                    mlp_ratio=mlp_ratio,
...
class PromptGenerator(nn.Module):
    ...
    def init_embeddings(self, x):
        N, C, H, W = x.permute(0, 3, 1, 2).shape # (B, C, H, W)
        x = x.reshape(N, C, H*W).permute(0, 2, 1) # (B, H*W, C)
    ...
    def get_prompt(self, handcrafted_feature, embedding_feature):
        N, C, H, W = handcrafted_feature.shape # (B, 24, H, W)
        handcrafted_feature = handcrafted_feature.view(N, C, H*W).permute(0, 2, 1) # (2, H*W, 24)
        prompts = []
        for i in range(self.depth): # unshared MLP
            lightweight_mlp = getattr(self, 'lightweight_mlp_{}'.format(str(i))) # unshared MLP
...
class TextPromptGenerator(nn.Module):
    def __init__(self, scale_factor, embed_dim, depth,
                 embedding_tune, adaptor):
        """
        Args:
        """
        super(TextPromptGenerator, self).__init__()
        self.scale_factor = scale_factor
        self.embed_dim = embed_dim
        self.depth = depth
        self.embedding_tune = embedding_tune
    ...
    def get_prompt(self, multimodal_features):
        prompts = []
        for i in range(self.depth): # unshared MLP
            lightweight_mlp = getattr(self, 'lightweight_mlp_{}'.format(str(i))) # unshared MLP
            # prompt = proj_prompt(prompt)
            prompt = lightweight_mlp(multimodal_features)
            prompts.append(self.shared_mlp(prompt))
        return prompts
