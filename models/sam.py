...
def _iou_loss(pred, target):
    ...
    inter = (pred * target).sum(dim=(2, 3)) # 交集
    union = (pred + target).sum(dim=(2, 3)) - inter # 元素集加法 - 交集
    iou = 1 - (inter / union)
...
class SAM(nn.Module):
    def __init__(self, inp_size=None, encoder_mode=None, loss=None):
        ...
        self.image_encoder = ImageEncoderViT(
            ...
            adapter_tuning=encoder_mode['adapter_tuning'],
            adapter_type=encoder_mode['adapter_type'],
            scale=encoder_mode['scale'],
        )
    ...
    def forward(self):
        ...
        # Embed prompts
        sparse_embeddings = torch.empty((bs, 0, self.prompt_embed_dim), device=self.input.device) # (1, 0 , 256)
    ...
    def backward_G(self):
        ...
        if self.loss_mode == 'iou':
            ...
            self.loss_G += _iou_loss(self.pred_mask, self.gt_mask)  # bce and iou ratio? 1:4/1:10?
