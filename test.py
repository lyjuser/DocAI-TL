...
#
#             # # 融合后的Feature map
#             # # feature_map, _ = torch.max(feature_map, dim=1, keepdim=True)  # (B, 1, 64, 64)
#             # fusion_feature_map = torch.mean(fusion_feature_map, dim=1, keepdim=True) # (B, 1, 64, 64)
#             # # print(feature_map.shape)
#             # fusion_feature_map = F.interpolate(fusion_feature_map, size=(1024, 1024), mode='bilinear', align_corners=False) # (B, 1, 1024, 1024)
#             # # print(feature_map.shape)
#             # fusion_feature_map = fusion_feature_map.squeeze(dim=1).cpu().detach().numpy() # (B, 1024, 1024)
#             # # print(fusion_feature_map)
#             # # feature_map = np.mean(feature_map, axis=1).reshape(1024, 1024)
#             # fusion_feature_map = (((fusion_feature_map - np.min(fusion_feature_map))/(np.max(fusion_feature_map)-np.min(fusion_feature_map)))*255).astype(np.uint8)
#             #
#             # fusion_feature_map = cv2.applyColorMap(fusion_feature_map[0], cv2.COLORMAP_JET)
#             # fusion_feature_map = cv2.cvtColor(fusion_feature_map, cv2.COLOR_BGR2RGB)
#             # fusion_feature_map = Image.fromarray(fusion_feature_map) # Numpy -> PIL
#             # # print(mask_path[0].split('/')[-1])
#             # save_fusion_feature_map_path = os.path.join(save_path, '{}_fusion_feature_map.png'.format(mask_path[0].split('/')[-1].split('.')[0]))
#             # # print("save_feature_map_path: ", save_feature_map_path)
#             # fusion_feature_map.save(save_fusion_feature_map_path)
#             # # cv2.imshow("feature_map", feature_map)
#             #
#             # # 融合前的Feature map
#             # feature_map = torch.mean(feature_map, dim=1, keepdim=True) # (B, 1, 64, 64)
#             # feature_map = F.interpolate(feature_map, size=(1024, 1024), mode='bilinear', align_corners=False) # (B, 1, 1024, 1024)
#             # feature_map = feature_map.squeeze(dim=1).cpu().detach().numpy() # (B, 1024, 1024)
#             # feature_map = (((feature_map - np.min(feature_map))/(np.max(feature_map)-np.min(feature_map)))*255).astype(np.uint8)
#             # feature_map = cv2.applyColorMap(feature_map[0], cv2.COLORMAP_JET)
#             # feature_map = cv2.cvtColor(feature_map, cv2.COLOR_BGR2RGB)
#             # feature_map = Image.fromarray(feature_map) # Numpy -> PIL
#             # save_feature_map_path = os.path.join(save_path, '{}_feature_map.png'.format(mask_path[0].split('/')[-1].split('.')[0]))
#             # feature_map.save(save_feature_map_path)
#
#             # 热力图
#             # feature_map, _ = torch.max(feature_map, dim=1, keepdim=True)  # (B, 1, 64, 64)
#             # fusion_feature_map = torch.mean(fusion_feature_map, dim=1, keepdim=True) # (B, 1, 64, 64)
...
#             # # feature_map = np.mean(feature_map, axis=1).reshape(1024, 1024)
#             # fusion_feature_map = (((fusion_feature_map - np.min(fusion_feature_map))/(np.max(fusion_feature_map)-np.min(fusion_feature_map)))*255).astype(np.uint8)
#             # plt.imshow(fusion_feature_map[0], cmap='Reds', origin='upper')
#             # save_fusion_feature_map_path = os.path.join(save_path, '{}_fusion_feature_map.png'.format(mask_path[0].split('/')[-1].split('.')[0]))
#             # plt.imsave(save_fusion_feature_map_path, fusion_feature_map[0], cmap='Reds')
#             #
#             # # 融合前
#             # feature_map = torch.mean(feature_map, dim=1, keepdim=True) # (B, 1, 64, 64)
#             # feature_map = F.interpolate(feature_map, size=(1024, 1024), mode='bilinear', align_corners=False) # (B, 1, 1024, 1024)
#             # feature_map = feature_map.squeeze(dim=1).cpu().detach().numpy() # (B, 1024, 1024)
#             # feature_map = (((feature_map - np.min(feature_map))/(np.max(feature_map)-np.min(feature_map)))*255).astype(np.uint8)
#             # plt.imshow(feature_map[0], cmap='Reds', origin='upper')
#             # save_feature_map_path = os.path.join(save_path, '{}_feature_map.png'.format(mask_path[0].split('/')[-1].split('.')[0]))
#             # plt.imsave(save_feature_map_path, feature_map[0], cmap='Reds')
#
#             # print(save_mode)
#             if save_mode == '1':
