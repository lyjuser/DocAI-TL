...
from models.multimodal import Visual_DocAI_Net
from PIL import Image
from transformers import AutoProcessor, AutoModel, AutoTokenizer, LayoutLMv3Processor
...
# def eval_psnr(loader, multimodal, eval_type=None):
#     multimodal.eval()
...
#
#         max_length3 = max(len(sub_l) for sub_l in batch_patch_original_bbox)
#         batch_patch_original_bbox_padding = [sub_l + [[0, 0, 0, 0]] * (max_length3 - len(sub_l)) for sub_l in batch_patch_original_bbox]  # Padding
...
#         inp = batch['inp_patch']
#
#         pred_mask = multimodal.infer(inp, input_ids, bbox, attention_mask, pixel_values, batch_patch_norm_bbox_padding, batch_patch_original_bbox_padding, visual_labels, offset_mapping)
...
def eval_psnr(loader, multimodal, processor, img_text_tampering_probs_dict, eval_type=None): # 读取CSV文件中存储的文本篡改概率
    ...
    for batch in pbar:
        ...
        pred_mask = multimodal.infer(inp, bbox=bbox, attention_mask=attention_mask, patch_norm_bbox=batch_patch_norm_bbox_padding, patch_original_bbox=batch_patch_original_bbox_padding,
                                     offset_mapping=offset_mapping, text_tampering_prob=batch_text_tampering_probs_padding)
        pred = torch.sigmoid(pred_mask)
...
# def train(train_loader, multimodal):
#     multimodal.train()
#
...
#         multimodal.optimize_parameters(input_ids, bbox, attention_mask, pixel_values, batch_patch_norm_bbox_padding, batch_patch_original_bbox_padding, visual_labels, offset_mapping)
#
#         train_loss_G.add(multimodal.loss_G.item())
#
...
def main(config_, save_path, args):
    ...
    multimodal = Visual_DocAI_Net(model, device=device)
    ...
    if config['DocAI'] == 'Layoutlmv3':
        ...
        processor = LayoutLMv3Processor.from_pretrained("./DocAI_pretrained/lilt", apply_ocr=False)
        ...
        if (epoch_val is not None) and (epoch % epoch_val == 0):
            ...
            if config['eval_type'] != 'ber': # 选择IoU
                if result3 > max_val_v:
                    max_val_v = result3
                    save(config, multimodal, save_path, 'best')
            else:
                if result3 < max_val_v:
                    max_val_v = result3
                    save(config, multimodal, save_path, 'best')
